...
class NotesService {
  ...
  //this getter function will return the  stream of data
  ...
  Future<DatabaseNotes> getNote({required int id}) async {
    ...
    if (notes.isEmpty) {
      ...
      //we might dhave outdated notes in our cache so we need to update it
  ...
  Future<void> deleteNote({required int id}) async {
    ...
    final deletedCount = await db.delete(
      noteTable,
      where:
          'id = ?', // "?" represents something and the something is passes in whereArgs
      whereArgs: [id],
    );
    if (deletedCount == 0) {
  ...
  Future<DatabaseNotes> createNote({required DatabaseUser owner}) async {
    ...
    _notes.add(note); //adding to the note
    _noteStreamController.add(_notes); //we add the data to the stream
    return note;
  }
